...
export function TaskDetailsPanel({ apiBase, readOnly, task, members, comments: initialComments }: TaskDetailsPanelProps) {
      ...
      <div className="space-y-6">
        <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
          ...
          <form onSubmit={saveTask} className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <button disabled={isSaving || readOnly} className="rounded-xl bg-violet-500 px-5 py-3 font-semibold transition hover:bg-violet-400 disabled:opacity-60 sm:col-span-2">{isSaving ? "Saving..." : "Save task"}</button>
            </fieldset>
          </form>
        </section>
      </div>
